...
class CAB(nn.Module):
    def __init__(self, num_feat, is_light= True, compress_ratio=3,squeeze_factor=30):
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.conv_blk = CAB(hidden_dim,is_light)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 downsample=None,
                 use_checkpoint=False,is_light=True):
        ...
        for i in range(depth):
            self.blocks.append(VSSBlock(
                ...
                input_resolution=input_resolution,is_light=is_light))
    ...
    def forward(self, x, x_size):
        for blk in self.blocks: #迭代blocks里的子模块，依次将数据传递计算
...
class Mamba_R(nn.Module):
    r""" Mamba_Reconstruction Model
    ...
    def __init__(self,
        ...
        self.upsample = Upsample(upscale, embed_dim, num_out_ch)

        self.apply(self._init_weights)

        self.mrb = MRB(dim=embed_dim, img_size=img_size, patch_size=patch_size)
    ...
    def forward_features(self, x, sample):
    ...
    def forward(self, x, xsample):
        self.mean = self.mean.type_as(x)
        x = (x - self.mean) * self.img_range

        x = self.conv_first(x)
        x = self.conv_after_body(self.forward_features(x, xsample)) + x
        x = self.upsample(x)
...
class MRB(nn.Module):
    ...
    def __init__(self,
        ...
        self.mix1 = nn.Sequential(
            nn.Conv2d(dim, 1, kernel_size=1, padding=0, stride=1, bias=True),
            nn.ReLU(),
            nn.Conv2d(1, dim, kernel_size=3, padding=1, stride=1, bias=True),
            nn.ReLU(),
        )
        self.mix2 = nn.Sequential(
            nn.Conv2d(dim, 4, kernel_size=1, padding=0, stride=1, bias=True),
            nn.ReLU(),
            nn.PixelShuffle(2),
        ...
        self.mix3 = nn.Sequential(
            nn.Conv2d(dim, 16, kernel_size=1, padding=0, stride=1, bias=True),
            nn.ReLU(),
            nn.PixelShuffle(4),
        ...
        self.mix4 = nn.Sequential(
            nn.Conv2d(dim, 64, kernel_size=1, padding=0, stride=1, bias=True),
            nn.ReLU(),
            nn.PixelShuffle(8),
    ...
    def forward(self, i, sample):
        if   i==1:
            sample = self.mix1(sample)
        elif i==2:
            sample = self.mix2(sample)
        elif i==3:
            sample = self.mix3(sample)
        elif i==4:
            sample = self.mix4(sample)

        sample = self.patch_embed(sample)
        return sample
...
class ResidualGroup(nn.Module):
    ...
    def __init__(self,
        ...
        # build the last conv layer in each group
